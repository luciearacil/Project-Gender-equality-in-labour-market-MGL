...
# graph of the evolution og Gap paid in Europe 
PC_Gapaid2 %>% 
...
# graph of the evolution of gap wages over tme
...
# Data of gap activity rate in 2018
...
PC_ActGapTime %>% 
  ...
  ylab("gap paid") +
  scale_color_discrete(label= c("Belgium"," Italy", "Lituania")) + theme_light() +
  theme(axis.text.x = element_text(angle=38))
...
# graph of the evolution of Italy  gap of activity rate 
PC_ActGapTime %>% 
...
# graph of the evolution of Turquey gap of activity rate 
PC_ActGapTime %>% 
  filter(country=="TR") %>% select(time,gap)%>%
  ggplot()+ geom_line(aes(x=time,y=gap,group=1),size=0.5) + labs(title="Evolution of Activity gap between wooman & men in Turquey") + xlab("time") + ylab("activity rate")
