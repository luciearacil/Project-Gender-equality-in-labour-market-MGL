...
## doc 1 (A REVOIR column values)
doc1 = doc1 %>% 
rename(country=geo, category=indic_em) 
  ### pourquoi pas créer 2 tableaux : un % actif et un % total employment, renommer la column value by % actif dans   le tableau 1 et la column value by % total employment dans le tableau 2, but = supprimer la colomne category et     que ce soit plus compréhensible quand on va tout regrouper 
  ### supprimer la colum % total employement parce que déjà présent dans tableau 2 (j'ai vérif et c'est les mêmes %)

## doc 2 (OK)
doc2 = doc2%>% 
rename(educ_level=isced11, country=geo, employement_rate=values)

## doc 3 (OK)
doc3 = doc3%>% 
rename(country=geo, parttime_rate_onemploy=values)
...
```{r}
# rename the label inside the columns
  
## doc 1 (OK)
doc1$category <- car::recode(doc1$category,
  '"ACT"="Actif"')
doc1$category <- car::recode(doc1$category,
  '"EMP_LFS"="Total_Employment"')
EmpAct=doc1
TA=doc1%>%filter(category=="Actif")
TA %>% group_by(country)

## doc 2 (OK)
doc2$educ_level<- car::recode(doc2$educ_level,'
  "ED0-2"="low";"ED3_4"="Inter";"ED5-8"="high"')
doc2=doc2 %>% filter(educ_level !="NRP")
